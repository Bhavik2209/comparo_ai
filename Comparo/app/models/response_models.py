...
class ComparisonResponse(BaseModel):
    """Response model for smartphone comparison."""
    success: bool = Field(..., description="Indicates if the operation was successful")
    message: str = Field(..., description="A descriptive message about the response")
    
    products: Optional[Union[List[Dict], Dict]] = Field(
    None, description="List of product schemas or a single product schema from the final state"
)
    best_product: Optional[Dict] = Field(None, description="Details of the best product")
    comparison: Optional[List[Dict]] = Field(None, description="Comparison details for products")
    youtube_link: Optional[str] = Field(None, description="A YouTube link for additional information or reviews")
